...
import { 
  ...
  Bell, Wallet, ShoppingBag, Calendar, CreditCard, Layers, ChevronRight, Globe, Loader2 
} from "lucide-react";
...
function DashboardContent({ children }: { children: React.ReactNode }) {
  ...
  const mainMenuItems = [
    ...
    { name: "My Profile", href: "/profile", icon: <User size={ICON_SIZE} />, exact: false },
  ];
        ...
        <header className="hidden md:flex items-center justify-between bg-white h-16 px-6 border-b border-gray-200 flex-shrink-0">
           ...
           <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 group relative">
                 ...
                 <button onClick={() => signOut({ callbackUrl: "http://localhost:3000" })} className="p-1.5 text-red-500 hover:bg-red-50 rounded-full ml-1 transition-colors" title="Sign Out">
                    <LogOut size={16} />
                 </button>
              </div>
           </div>
